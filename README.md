...
长会话接近上下文预算时，Runtime 会自动压缩较早轮次：保留稳定的系统提示和最近的完整用户轮次，将较早对话、工具调用与结果整理为结构化摘要。对使用业务空间专属域名的 OpenAI-Compatible/Qwen 配置，Runtime 会尝试查询模型元数据，并在估算上下文达到模型窗口的 85% 时触发；查询失败或未返回窗口大小时回退到 100,000 Token。压缩以完整轮次为边界，不会拆开 `tool_call`/`tool` 消息；摘要失败时保留原始历史并继续用户请求。界面仍保存完整可见对话，只有发给模型的 Agent 历史会被压缩，压缩请求也不会计入正常对话的缓存命中率。
