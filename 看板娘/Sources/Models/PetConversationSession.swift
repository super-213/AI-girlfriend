...
struct PetConversationSession {
    ...
    mutating func expireIfNeeded(at date: Date, timeout: TimeInterval?) {
        guard let timeout, let lastConversationAt,
              date.timeIntervalSince(lastConversationAt) >= timeout else { return }
        destroy()
    }

    func remainingLifetime(at date: Date, timeout: TimeInterval?) -> TimeInterval? {
        guard let timeout, let lastConversationAt, !history.isEmpty else { return nil }
        return max(timeout - date.timeIntervalSince(lastConversationAt), 0)
    }
